...
library Events {
    ...
    event PostionRegistered(
        address sender, uint256 tokenId, uint256 deadline, address pool, uint256 presaleAmount, uint256 mintAmount
    );
    event GuardVaultIsSet(address pre, address current);
    event PrincipleGuardDeployed(address guard, address yield, int24 floorTick, uint256 bal);
    event ManagerUpdated(address _manager);
    //guard event
    event MintBase(uint256 anchorTokenId, int24 anchorTickLower, int24 anchorTickUpper);
    event MintAscent(uint256 ascentTokenId, int24 ascentTickLower, int24 ascentTickUpper);
    ...
    event YieldDistributed(
        uint256 indexed tokenId, uint256 holderShare, uint256 baselineShare, uint256 platformShare, uint256 timestamp
    );
