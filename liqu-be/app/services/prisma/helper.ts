...
const PrismaHelper = {
  buildQuery: function (request: FastifyRequest, allowedFields: string[] = []) {
    const { filterKeys = '', filterOperators = '', filterValues = '' } = request.query as any
    const operatorDicts = {
      '=': 'equals',
      '!=': 'not',
      '>': 'gt',
    ...
    if (keys.length !== operators.length || operators.length !== values.length) {
      throw new AppException(400, ErrorCodes.BAD_REQUEST, 'Invalid filter queries')
    }

    keys.forEach((key, i) => {
      if (allowedFields.includes(key) && Object.keys(operatorDicts).includes(operators[i])) {
